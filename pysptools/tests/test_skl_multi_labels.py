...
#------------------------------------------------------------------------------
#
# test_sklearn_multi_labels.py - This file is part of the PySptools package.
#
...
def test_HyperSVC(path, data, r):
    c_w = {0:1,1:10,2:10,3:10,4:10,5:10,6:10,7:10,8:10,9:10,10:10,11:10,12:10,13:10,14:10}
    model = skl.HyperSVC(class_weight=c_w)
    model.fit_rois(data, r)
    model.classify(data)
    model.plot(path, labels=r.get_labels(), interpolation=None, colorMap='Paired')


def test_HyperGaussianNB(path, data, r):
#    data_scaled = hyper_scale(data)
    data_scaled = data
    model = skl.HyperGaussianNB()
    model.fit_rois(data_scaled, r)
    model.classify(data_scaled)
    model.plot(path, labels=r.get_labels(), interpolation=None, colorMap='Paired')

    
def test_HyperKNeighborsClassifier(path, data, r):
#    data_scaled = hyper_scale(data)
    data_scaled = data
    model = skl.HyperKNeighborsClassifier(n_neighbors=4, algorithm='ball_tree', leaf_size=10)
    model.fit_rois(data_scaled, r)
    model.classify(data_scaled)
    model.plot(path, labels=r.get_labels(), interpolation=None, colorMap='Paired')

    
def test_HyperLogisticRegression(path, data, r):
    # better when not scaled
    data_scaled = skl.hyper_scale(data)
#    data_scaled = data
    c_w = {0:1,1:10,2:10,3:10,4:10,5:10,6:10,7:10,8:10,9:10,10:10,11:10,12:10,13:10,14:10}
    model = skl.HyperLogisticRegression(class_weight=c_w)
    model.fit_rois(data_scaled, r)
    model.classify(data_scaled)
    model.plot(path, labels=r.get_labels(), interpolation=None, colorMap='Paired')

    
def test_HyperRandomForestClassifier(path, data, r):
    model = skl.HyperRandomForestClassifier(n_estimators=10)
    #model = HyperRandomForestClassifier(n_estimators=100, class_weight=c_w)
    model.fit_rois(data, r)
    model.classify(data)
    model.plot(path, labels=r.get_labels(), interpolation=None, colorMap='Paired')
